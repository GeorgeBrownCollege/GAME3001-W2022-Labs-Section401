...
void Tile::addLabels()
{
	Config::OFFSET = glm::vec2(Config::TILE_SIZE * 0.5f, Config::TILE_SIZE * 0.5f);

	// cost label
	m_costLabel = new Label("99.9", "Consolas", 12);
	m_costLabel->getTransform()->position = getTransform()->position + Config::OFFSET + glm::vec2(0.0f, -6.0f);
	getParent()->addChild(m_costLabel);
	m_costLabel->setEnabled(false);

	// status label
	m_statusLabel = new Label("=", "Consolas", 12);
	m_statusLabel->getTransform()->position = getTransform()->position + Config::OFFSET + glm::vec2(0.0f, 6.0f);
